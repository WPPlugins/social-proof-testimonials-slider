...
class Social_Proof_Slider_Shared {
		...
		if ( is_wp_error( $query ) ) {
			...
			if ( $query->have_posts() ) :
				...
				while ( $query->have_posts() ) : $query->the_post();
					...
					$surroundWithQuotes = false; // default
					if ( $smartquotes == 1 ){
				...
				$output .= 'No Testimonials found. <a href="' . $bloghome . '/wp-admin/post-new.php?post_type=socialproofslider">Add one now!</a>';
	...
	public function get_shortcode_settings() {
		...
		$surroundWithQuotes = true;  // default
		if ( empty( $surroundWithQuotes ) || $surroundWithQuotes == '' ){
			$surroundWithQuotes = false;
		}
		$args['surroundWithQuotes'] = $surroundWithQuotes;
